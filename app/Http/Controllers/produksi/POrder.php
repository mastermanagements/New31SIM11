...
use App\Http\utils\data_pembelian\PesananPembelian as data_pesanan_pembelian;
...
//          return redirect('Pembelian')->with('message_success', 'anda telah mengubah rincian pembelian')->with('tab3','tab3');
    ...
    public function laporan_penasan_pembelian(Request $req)
    {
        $pesanan_pembelian_class = new data_pesanan_pembelian();
        $data_pesanan_pembelian = $pesanan_pembelian_class->get_data($req);
        $supplier = Supplier::all()->where('id_perusahaan', Session::get('id_perusahaan_karyawan'));
        if ($req->action == 'preview') {
            return view('user.produksi.section.laporan.pesanan_pembelian.page_show', ['data' => $data_pesanan_pembelian, 'supplier' => $supplier]);
        } elseif ($req->action == 'print') {
            $header = HeaderReport::header_format_2('layouts.header_print.header_print1', 'LAPORAN PESANAN PEMBELIAN');
            return view('user.produksi.section.laporan.pesanan_pembelian.cetak', ['data' => $data_pesanan_pembelian, 'header' => $header]);
        } else {
    ...
    public function laporan_pembelian_barang(Request $req)
    {
        $pesanan_pembelian_class = new data_pesanan_pembelian();
        $data_pembelian_barang = $pesanan_pembelian_class->get_data_pembelian($req);
        $supplier = Supplier::all()->where('id_perusahaan', Session::get('id_perusahaan_karyawan'));
        if ($req->action == 'preview') {
            return view('user.produksi.section.laporan.pembelian_barang.page_show', ['data' => $data_pembelian_barang, 'supplier' => $supplier]);
        } elseif ($req->action == 'print') {
            $header = HeaderReport::header_format_2('layouts.header_print.header_print1', 'LAPORAN PEMBELIAN BARANG');
            return view('user.produksi.section.laporan.pembelian_barang.cetak', ['data' => $data_pembelian_barang, 'header' => $header]);
        } else {
            return view('user.produksi.section.laporan.pembelian_barang.page_show', ['data' => $data_pembelian_barang, 'supplier' => $supplier]);
        }
    }

    public function laporan_Hutang_Pembelian(Request $req)
    {
        $pesanan_hutang_class= new HutangPembelian();
        $data_hutang_pembelian_barang = $pesanan_hutang_class->data($req);
        $supplier = Supplier::all()->where('id_perusahaan', Session::get('id_perusahaan_karyawan'));
        if ($req->action == 'preview') {
            ...
            $header = HeaderReport::header_format_2('layouts.header_print.header_print1', 'LAPORAN HUTANG PEMBELIAN BARANG');
