...
class Pembelian
{
    ...
    public static function getData()
    {
        ...
        foreach ($pembelian->get() as $item) {
            ...
            $column[] = $item->ongkir;
            $tgl_tiba = "";
            if ($item->tgl_tiba != '1970-01-01') {
                $tgl_tiba = date('d-m-Y', strtotime($item->tgl_tiba));
            }
            $column[] = $tgl_tiba;
            ...
            if ($item->tgl_jatuh_tempo != "1970-01-01") {
                $tbl_jatuh_tempo = date('d-m-Y', strtotime($item->tgl_jatuh_tempo));
            }
            $column[] = $tbl_jatuh_tempo;
            $data[] = $column;
        }
        return $data;
    }
    ...
    private static function list_detail_barang($detail_order)
    {
        ...
        if (!empty($detail_order->linkToDetailOrder)) {
            foreach ($detail_order->linkToDetailOrder as $item_detail_order) {
                ...
                $tgl_expired_date = "";
                if (($item_detail_order->expired_date != "1970-01-01") && $item_detail_order->expired_date != null) {
                    $tgl_expired_date = date('d-m-Y', strtotime($item_detail_order->expired_date));
                }
                $column[] = $tgl_expired_date;
                $contaier[] = $column;
            }
        }
        return $contaier;
