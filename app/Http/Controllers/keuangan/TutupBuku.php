...
class TutupBuku extends Controller
{
    ...
    public function store(Request $req){
        $yearsNow = date('Y');
        $no_transaksi=1;
        if($yearsNow != $req->thn_periode){
            return redirect('neraca')->with('message_error_tutup_buku','Tahun Periode tidak sama dengan tahun berjalan');
        }else{
                foreach ($req->id_aktif_ukm as $key=>$data){
                    ...
                    if($explode_tgl[0] == $req->thn_periode){
                ...
                return redirect('neraca')->with('message_success_tutup_buku','Proses tutup buku selesai');
        }
    }
}
