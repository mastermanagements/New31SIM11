                ...
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
						<div class="tab-pane active" id="tab_1">
							<div class="col-md-12">
									<label style="font-size: 15px">Pelaksanaan Marketing - Attract Tahun {{now()->year }}</label>
							</div>
							</br></br>
							
							<!--Attract Barang Off Line-->
																			...
																			@foreach($keg_market_harian as $km_harian)
																				@if($km_harian->id_pel_m == $pel_markets->id) 
																					@if($km_harian->getKegMarketing->jenis_keg_marketing == "Publish")
																						<button type="button" class="btn btn-primary" onclick="tambahResponAttract('{{ $pel_markets->id }}');" title="Tambah Respon Leads"><i class="fa fa-plus"></i></button>
																					@endif
																				@endif
																			@endforeach
																		</td>
													...
													@foreach($pel_marketing as $pel_markets)
														@if($rm_fase_j->id_rm == $rm_off->id)
															@if($rm_fase_j->id == $pel_markets->id_rm_fase) 
																@if($rm_fase_j->fase_marketing == "Attract (Pengenalan)")
																	<tr>
																		<td>{{ $pel_markets->id}}</td>
																		<td>{{ date('d-m-Y h:i:s', strtotime($pel_markets->created_at ))}} </td>
...
@include('user.marketing.section.pelaksanaan_marketing.attract.modal_respon_attract')
@stop
@section('plugins')
	...
	<script>
		...
		$(document).ready(function () {
			...
			tambahResponAttract = function (id) {
			//alert("test")
				$('[name="id_pel_m"]').val(id);
                $('#modal-tambah-ResponAttract').modal('show');
            };
		})
	 </script>
@stop
