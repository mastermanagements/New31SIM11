        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    <!-- /.box-header -->
                    <!-- form start -->
                    <form role="form" action="{{ url('akun-pembelian') }}" method="post" enctype="multipart/form-data">
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="jenis_jurnal">
                                    ...
                                    @foreach($jenis_jurnal as $key=> $jenis_jur)
                                        <option value="{{ $key }}">{{ $jenis_jur }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="id_ket_transaksi">
                                    ...
                                    @foreach($keterangan_transaksi as $key=> $n_transaksi)
                                        <option value="{{ $n_transaksi->id }}">{{ $n_transaksi->nm_transaksi }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
