            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box box-warning">
                        ...
                        <!-- /.box-header -->
                        <!-- form start -->
                        <form role="form" action="{{ url('update-proyek/'. $data->id) }}" method="post" enctype="multipart/form-data">
                            <div class="box-body">
                                    ...
                                    <select class="form-control select2" style="width: 100%;" name="jenis_proyek" required>
                                        ...
                                        @else
                                            @foreach($jenis_proyek as $key=> $value)
                                                <option value="{{ $key }}" @if($data->jenis_proyek == $key) selected @endif>{{ $value }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    ...
                                    <select class="form-control select2" style="width: 100%;" name="id_spk" required>
                                        ...
                                        @else
                                            @foreach($spk as $value)
                                                <option value="{{ $value->id }}" @if($value->id == $data->id_spk) selected @endif>{{ $value->nm_spk }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    <small style="color: red">* Tidak Boleh Kosong</small>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" min="0"  name="jangka_waktu" class="form-control" placeholder="Lama Proyek" value="{{ $data->jangka_waktu }}" required/>
                                    <small style="color: red">* Tidak boleh kosong</small>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1">Detail Proyek</label>
                                    <textarea name="detail_proyek" class="form-control" required>{!! $data->rincian_proyek !!}</textarea>
                                    <small style="color: red">* Tidak Boleh Kosong</small>
                                </div>
