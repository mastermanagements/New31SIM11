            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box box-warning">
                        ...
                        <div class="box-body">
                            <div class="row">
                                <form role="form" action="{{ url('produksi-baru/'.$data_produksi->id) }}" method="post" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    @method('put')
                                    <div class="col-md-12">
                                            ...
                                            <select class="form-control select2" name="id_barang" style="width: 100%">
                                                ...
                                                @if(!empty($barang_jadi))
                                                    @foreach($barang_jadi as $data_barang_jadi)
                                                        <option value="{{ $data_barang_jadi->id }}" @if($data_produksi->id_barang== $data_barang_jadi->id) selected @endif> {{ $data_barang_jadi->nm_barang }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                            ...
                                            <select class="form-control select2" name="brg_dalam_proses" style="width: 100%">
                                                ...
                                                @if(!empty($barang_dalam_proses))
                                                    @foreach($barang_dalam_proses as $barang_dalam_proses_item)
                                                        <option value="{{ $barang_dalam_proses_item->id }}" @if($data_produksi->id_barang==$barang_dalam_proses_item->id) selected @endif> {{ $barang_dalam_proses_item->nm_barang }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="kode_produksi" value="{{ $kode_produksi }}" disabled>
                                        </div>
                                        <div class="form-group">
                                            <label>Batch Number</label>
                                            <input type="text" class="form-control" name="batch_number" value="{{ $data_produksi->batch_number }}">
                                        </div>
                                        <div class="form-group">
                                            <label>No Serial</label>
                                            <input type="text" class="form-control" name="no_serial" value="{{ $data_produksi->no_serial }}">
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="id_supervisor_produksi" style="width: 100%">
                                                ...
                                                @if(!empty($supervisor))
                                                    @foreach($supervisor as $data_supervisor)
                                                        <option value="{{ $data_supervisor->id }}" @if($data_produksi->id_supervisor_produksi == $data_supervisor->id) selected @endif> {{ $data_supervisor->nama_ky }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
